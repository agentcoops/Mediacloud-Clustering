...
# $dbs->query("UPDATE downloads SET state = 'error', error_message = 'removed from queue at startup' where (state = 'queued' or state='pending') and type = 'feed'");
        ...
        $dbs->query("UPDATE downloads set state = 'pending' where state = 'queued' and type in  ('spider_blog_home','spider_posting','spider_rss')");
...
"update downloads set (state,error_message) = ('pending','') where state='error' and type in ('spider_blog_home','spider_posting','spider_rss') and (error_message like '50%' or error_message = '$download_timed_out_error_message') ;"
    );
